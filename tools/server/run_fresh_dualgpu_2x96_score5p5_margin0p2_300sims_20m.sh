...
TRAIN_OUT="artifacts/multiworker-9x9-fresh-dualgpu-2x96-score5p5-margin0p2-300sims-max150-20m-20260607"
LOG="$TRAIN_OUT/train.log"
MONITOR="$TRAIN_OUT/gpu_monitor.csv"

mkdir -p "$TRAIN_OUT"
cat > "$TRAIN_OUT/run_notes.md" <<NOTES
# Fresh Dual-GPU 2x96, Score Komi 5.5, Margin Reward 0.2, 20-Minute Throughput Check
...
- purpose: short throughput/comparison check before any long run
- fresh_start: true
- comparison_warning: raw cycle numbers are not directly comparable with earlier runs because games_per_cycle changed
- previous_4x64_games_per_cycle: 32
- this_2x96_dualgpu_games_per_cycle: 96
- preferred_comparison_units: total positions, total games, wall-clock time, and eval results rather than cycle ID
