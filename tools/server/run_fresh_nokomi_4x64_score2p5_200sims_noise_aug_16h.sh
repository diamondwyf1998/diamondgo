...
RESUME="artifacts/multiworker-9x9-fresh-nokomi-4x64-score2p5-100sims-noise-aug-2h-20260605/latest.pt"
LOG="$TRAIN_OUT/train.log"
MONITOR="$TRAIN_OUT/gpu_monitor.csv"

mkdir -p "$TRAIN_OUT"
cat > "$TRAIN_OUT/run_notes.md" <<NOTES
# Fresh No-Komi-Input 4x64, 200-Sim Overnight Continuation
...
- reason: test whether increasing MCTS search from 100 to 200 improves tactical/shape learning without changing the architecture
