...
RESUME="artifacts/multiworker-9x9-fresh-nokomi-4x64-score4p5-200sims-noise-aug-6h-20260606/latest.pt"
LOG="$TRAIN_OUT/train.log"
MONITOR="$TRAIN_OUT/gpu_monitor.csv"

mkdir -p "$TRAIN_OUT"
cat > "$TRAIN_OUT/run_notes.md" <<NOTES
# Fresh No-Komi-Input 4x64, Score Komi 6.5 Continuation
...
- reason: score_komi=4.5 latest self-play black win rate remained above 75%, so score komi is raised to 6.5
