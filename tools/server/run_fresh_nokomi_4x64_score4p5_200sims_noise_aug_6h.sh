...
RESUME="artifacts/multiworker-9x9-fresh-nokomi-4x64-score2p5-200sims-noise-aug-16h-20260606/latest.pt"
LOG="$TRAIN_OUT/train.log"
MONITOR="$TRAIN_OUT/gpu_monitor.csv"

mkdir -p "$TRAIN_OUT"
cat > "$TRAIN_OUT/run_notes.md" <<NOTES
# Fresh No-Komi-Input 4x64, Score Komi 4.5 Continuation
...
- reason: previous score_komi=2.5 continuation remained strongly Black-skewed, so score komi is raised to 4.5
