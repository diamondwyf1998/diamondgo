...
- Self-play review can save the currently displayed position as a tactical
  ...
  after the current step; `to_play` is the next player; if a next self-play move
  exists, it becomes the initial `good` target. The self-play viewer exposes a
...
- Evaluation dashboards generated by `src/diamondgo/eval_checkpoints.py` use
  ...
  the selected move, set `category` to `eval_match_snapshot`, and use the next
  match move as the initial `good` target when one exists.
